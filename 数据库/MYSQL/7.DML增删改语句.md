...
* 在 INSERT 语句中加入子查询。 
* 不必书写 VALUES 子句。 
* 子查询中的值列表类型应与 INSERT 子句中的列名类型对应。
* 若查询出来的类型不能赋值给添加列的类型，或者长度超出添加列的长度，将会失败，所以该方式使用具有一定风险
...
* 可以一次更新多条数据。
* **如果需要回滚数据,不自动提交数据，需要保证在DML前，进行设置：`SET AUTOCOMMIT = FALSE;`**
...
如果使用UPDATE关键字更新多行，发生错误，使用`IGNORE`关键字可以继续进行更新
...
DELETE u,m 
FROM users u JOIN my_employees m ON u.userid = m.userid
...
```mysql
INSERT INTO tb1(a,b) VALUES(100,2030)
```
